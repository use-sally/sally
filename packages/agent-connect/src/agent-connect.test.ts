...
test('public Hermes command parses npx-style runtime subcommand and pairing flags', async () => {
...
test('public help teaches first-time Hermes users that Sally supplies the connector', async () => {
  ...
  assert.match(help, /Hermes does not need to know Sally/i)
  assert.match(help, /Sally connector/i)
})

test('unsupported runtimes fail before doing any Sally or Hermes work', async () => {
  const { parseAgentConnectArgs } = await import('./cli-options.js')

  assert.throws(
    () => parseAgentConnectArgs(['codex', '--pairing-code', 'ABCD-EFGH'], { HOME: '/tmp/sally-home' }),
