...
test('workflow start control is a human-readable status/action surface', () => {
  assert.match(automationControlsSource, /type WorkflowControlTone = 'neutral' \| 'active' \| 'success' \| 'attention' \| 'danger'/)
  assert.match(automationControlsSource, /type WorkflowControlAction = 'connect' \| 'start' \| 'open-automation'/)
  ...
  assert.match(automationControlsSource, /pendingApproval[\s\S]*label: 'Approval needed'[\s\S]*tone: 'attention'[\s\S]*Sally is waiting for your decision/)
  assert.match(automationControlsSource, /openBlocker[\s\S]*label: 'Blocked: needs input'[\s\S]*tone: 'danger'[\s\S]*Sally cannot continue until this blocker is resolved/)
  assert.match(automationControlsSource, /latestTerminalStatus === 'BLOCKED'[\s\S]*label: 'Blocked: needs input'[\s\S]*tone: 'danger'/)
  assert.match(automationControlsSource, /\['FAILED', 'TIMED_OUT', 'CANCELLED'\]\.includes\(latestTerminalStatus\)[\s\S]*label: 'Workflow failed'[\s\S]*tone: 'danger'[\s\S]*Review the last run, then retry planning/)
  assert.match(automationControlsSource, /latestTerminalStatus === 'SUCCEEDED' && workflowEnabled[\s\S]*label: 'All caught up'[\s\S]*tone: 'success'[\s\S]*no queued, running, blocked, or waiting workflow work/)
  assert.match(automationControlsSource, /workflowEnabled \? 'Plan again' : 'Start planning'/)
  assert.match(automationControlsSource, /style=\{automationIslandControlStyle\(workflowControl\.tone\)\}/)
  assert.match(automationControlsSource, /automationStatusHelperStyle\(workflowControl\.tone, compact\)/)
  assert.match(automationControlsSource, /router\.replace\(`\/projects\/\$\{projectId\}\?view=automation`, \{ scroll: false \}\)/)
  assert.doesNotMatch(automationControlsSource, /Waiting on blocker/)
  assert.doesNotMatch(automationControlsSource, /Last workflow failed/)
  assert.doesNotMatch(automationControlsSource, /Ready: workflow enabled/)
  assert.doesNotMatch(automationControlsSource, /Ready to plan/)
})

test('project automation toggles use island switcher styling instead of toggle pills', () => {
  assert.match(automationControlsSource, /function automationIslandControlStyle\(activeOrTone: boolean \| WorkflowControlTone, danger = false\)/)
  assert.match(automationControlsSource, /function workflowToneStyle\(tone: WorkflowControlTone\)/)
  ...
  assert.match(automationControlsSource, /tone === 'danger'[\s\S]*rgba\(239, 68, 68, 0\.12\)/)
  assert.match(automationControlsSource, /tone === 'attention'[\s\S]*rgba\(245, 158, 11, 0\.14\)/)
  assert.match(automationControlsSource, /tone === 'success'[\s\S]*rgba\(34, 197, 94, 0\.12\)/)
  assert.match(automationControlsSource, /border:\s*danger \? '1px solid var\(--danger-text\)' : toneStyle\.border/)
  assert.doesNotMatch(automationControlsSource, /toggleKnob/)
  const automationControlStyle = automationControlsSource.match(/function automationIslandControlStyle\(activeOrTone: boolean \| WorkflowControlTone, danger = false\): CSSProperties \{[\s\S]*?\n\}/)?.[0] || ''
