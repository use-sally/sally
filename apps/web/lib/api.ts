...
export function getProfile(): Promise<{ ok: boolean; profile: { id: string; name: string | null; email: string; avatarUrl: string | null; pendingEmail: string | null; platformRole?: 'NONE' | 'ADMIN' | 'SUPERADMIN'; appearanceFontScale?: number; appearanceTheme?: 'dark' | 'light' } }> { return getJson('/auth/profile') }
export function updateProfile(payload: { name?: string; email?: string; avatarUrl?: string | null; appearanceFontScale?: number; appearanceTheme?: 'dark' | 'light' }): Promise<{ ok: boolean; profile: { id: string; name: string | null; email: string; avatarUrl: string | null; platformRole?: 'NONE' | 'ADMIN' | 'SUPERADMIN'; appearanceFontScale?: number; appearanceTheme?: 'dark' | 'light' }; emailChange?: { pendingEmail: string; emailed: boolean; reason?: string } | null }> { return getJson('/auth/profile', { method: 'PATCH', body: JSON.stringify(payload) }) }
