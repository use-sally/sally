...
export function createAgentPairingCode(payload?: { projectId?: string | null; agentId?: string | null; name?: string | null; runtimeType?: string | null; ttlMinutes?: number | null }): Promise<{ ok: boolean; pairingCode: string; expiresAt: string; pairingId: string }> { return getJson('/agent-connections/pairing-code', { method: 'POST', body: JSON.stringify(payload || {}) }) }
export function revokeAgentConnection(connectionId: string, payload?: { clearQueue?: boolean }): Promise<{ ok: boolean; clearQueue?: boolean; cancelledJobs?: number; cancelledRuns?: number }> { return getJson(`/agent-connections/${connectionId}/revoke`, { method: 'POST', body: JSON.stringify(payload || {}) }) }
...
export function createProjectStatus(projectId: string, payload: { name: string; type: 'BACKLOG' | 'TODO' | 'IN_PROGRESS' | 'BLOCKED' | 'REVIEW' | 'DONE' }): Promise<{ ok: boolean; statusId: string }> { return getJson(`/projects/${projectId}/statuses`, { method: 'POST', body: JSON.stringify(payload) }) }
export function updateProjectStatus(projectId: string, statusId: string, payload: { name?: string; type?: 'BACKLOG' | 'TODO' | 'IN_PROGRESS' | 'BLOCKED' | 'REVIEW' | 'DONE'; color?: string }): Promise<{ ok: boolean }> { return getJson(`/projects/${projectId}/statuses/${statusId}`, { method: 'PATCH', body: JSON.stringify(payload) }) }
...
export function reorderProjectStatuses(projectId: string, orderedStatusIds: string[]): Promise<{ ok: boolean }> { return getJson(`/projects/${projectId}/statuses/reorder`, { method: 'POST', body: JSON.stringify({ orderedStatusIds }) }) }
...
export function getProfile(): Promise<{ ok: boolean; profile: { id: string; name: string | null; email: string; avatarUrl: string | null; pendingEmail: string | null; platformRole?: 'NONE' | 'ADMIN' | 'SUPERADMIN'; appearanceFontScale?: number; appearanceTheme?: 'dark' | 'light'; appearanceStatusTint?: number } }> { return getJson('/auth/profile') }
export function updateProfile(payload: { name?: string; email?: string; avatarUrl?: string | null; appearanceFontScale?: number; appearanceTheme?: 'dark' | 'light'; appearanceStatusTint?: number }): Promise<{ ok: boolean; profile: { id: string; name: string | null; email: string; avatarUrl: string | null; platformRole?: 'NONE' | 'ADMIN' | 'SUPERADMIN'; appearanceFontScale?: number; appearanceTheme?: 'dark' | 'light'; appearanceStatusTint?: number }; emailChange?: { pendingEmail: string; emailed: boolean; reason?: string } | null }> { return getJson('/auth/profile', { method: 'PATCH', body: JSON.stringify(payload) }) }
