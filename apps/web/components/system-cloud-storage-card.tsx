    ...
    <div role="presentation" style={modalBackdrop} onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-labelledby="cloud-storage-integrations-title" style={modalPanel} onClick={(event) => event.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start', padding: '18px 20px', borderBottom: '1px solid var(--panel-border)', flex: '0 0 auto' }}>
          <div>
            <h2 id="cloud-storage-integrations-title" style={{ margin: 0, color: 'var(--text-primary)', fontSize: 18 }}>Cloud storage integrations</h2>
    ...
    <>
      <button type="button" onClick={() => setModalOpen(true)} aria-pressed={modalOpen} style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}>
          ...
          <span style={{ border: '1px solid rgba(250,204,21,0.32)', borderRadius: 999, padding: '4px 8px', color: 'var(--task-title)', fontSize: 11, fontWeight: 700 }}>Enterprise</span>
        </div>
        <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.55 }}>{description}</p>
        <span style={{ color: 'var(--task-title)', fontSize: 12, fontWeight: 750 }}>{modalOpen ? 'Modal open' : 'Open modal'} →</span>
      </button>
