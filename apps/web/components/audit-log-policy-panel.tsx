...
export function AuditLogPolicyPanel() {
  ...
  return (
    <section style={{ border: '1px solid var(--panel-border)', borderRadius: 16, background: 'var(--panel-bg)', padding: 18, display: 'grid', gap: 12 }}>
      <div>
        <h3 style={{ margin: 0, color: 'var(--task-title)', fontSize: 16 }}>Audit log</h3>
        <p style={{ margin: '6px 0 0', color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.55 }}>Enterprise audit retention, export, and visibility controls.</p>
      </div>
      ...
      <form onSubmit={submit} style={{ display: 'grid', gap: 12 }}>
        ...
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 13 }}><input type="checkbox" checked={exportRequiresAdmin} onChange={(event) => setExportRequiresAdmin(event.target.checked)} /> Require platform admin role for CSV export</label>
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 13 }}><input type="checkbox" checked={includeAuthEvents} onChange={(event) => setIncludeAuthEvents(event.target.checked)} /> Include authentication events in default audit views</label>
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 13 }}><input type="checkbox" checked={includeAutomationEvents} onChange={(event) => setIncludeAutomationEvents(event.target.checked)} /> Include automation events in default audit views</label>
...
function Stat({ label, value }: { label: string; value: number }) {
  return <div style={{ border: '1px solid var(--panel-border)', borderRadius: 12, background: 'var(--form-bg)', padding: 12 }}><div style={{ color: 'var(--text-secondary)', fontSize: 12 }}>{label}</div><div style={{ color: 'var(--text-primary)', fontSize: 18, fontWeight: 800 }}>{value}</div></div>
}
