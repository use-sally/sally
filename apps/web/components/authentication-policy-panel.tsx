  ...
  return (
    <section style={{ border: '1px solid var(--panel-border)', borderRadius: 16, background: 'var(--panel-bg)', padding: 18, display: 'grid', gap: 12 }}>
      ...
      <form onSubmit={submit} style={{ display: 'grid', gap: 12 }}>
        ...
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 'var(--font-13)' }}><input type="checkbox" checked={requirePasswordUppercase} onChange={(event) => setRequirePasswordUppercase(event.target.checked)} /> Require uppercase letters</label>
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 'var(--font-13)' }}><input type="checkbox" checked={requirePasswordLowercase} onChange={(event) => setRequirePasswordLowercase(event.target.checked)} /> Require lowercase letters</label>
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 'var(--font-13)' }}><input type="checkbox" checked={requirePasswordNumber} onChange={(event) => setRequirePasswordNumber(event.target.checked)} /> Require numbers</label>
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 'var(--font-13)' }}><input type="checkbox" checked={requirePasswordSymbol} onChange={(event) => setRequirePasswordSymbol(event.target.checked)} /> Require symbols</label>
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 'var(--font-13)' }}><input type="checkbox" checked={disablePasswordLoginForSso} onChange={(event) => setDisablePasswordLoginForSso(event.target.checked)} /> Disable password login for non-superadmins when SSO is enabled</label>
