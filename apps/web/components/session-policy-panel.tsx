...
export function SessionPolicyPanel() {
  ...
  return (
    <section style={{ border: '1px solid var(--panel-border)', borderRadius: 16, background: 'var(--panel-bg)', padding: 18, display: 'grid', gap: 12 }}>
      <div>
        <h3 style={{ margin: 0, color: 'var(--task-title)', fontSize: 16 }}>Sessions</h3>
        <p style={{ margin: '6px 0 0', color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.55 }}>Enterprise session lifetime and force-logout controls. New sessions use the configured lifetime.</p>
      </div>
      <div style={{ border: '1px solid var(--panel-border)', borderRadius: 12, background: 'var(--form-bg)', padding: 12, color: 'var(--text-secondary)', fontSize: 13 }}>Active sessions: <strong style={{ color: 'var(--text-primary)' }}>{activeSessions}</strong></div>
      ...
      <form onSubmit={submit} style={{ display: 'grid', gap: 12 }}>
        ...
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 13 }}><input type="checkbox" checked={revokeOnPolicyChange} onChange={(event) => setRevokeOnPolicyChange(event.target.checked)} /> Revoke other active sessions when this policy changes</label>
        <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: 'var(--text-secondary)', fontSize: 13 }}><input type="checkbox" checked={restrictSessionPolicyToAdmins} onChange={(event) => setRestrictSessionPolicyToAdmins(event.target.checked)} /> Restrict session policy management to platform admins</label>
