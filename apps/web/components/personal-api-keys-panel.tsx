  ...
  return (
    <div style={{ ...panel, display: 'grid', gap: 12 }}>
      <div style={{ display: 'grid', gap: 4 }}>
        ...
        <div style={{ color: '#64748b', fontSize: 13 }}>Create your own key to use the PM tool from external clients. Keys are only shown once when created.</div>
      </div>
      ...
      {apiKeySecret ? (
        <div style={{ display: 'grid', gap: 8, padding: 12, borderRadius: 14, background: '#f8fafc', border: '1px solid #e2e8f0' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#64748b', textTransform: 'uppercase' }}>New API key</div>
          <code style={{ wordBreak: 'break-all', fontSize: 13 }}>{apiKeySecret}</code>
          <div>
            <button onClick={() => void handleCopyKey()} style={{ borderRadius: 10, border: '1px solid #dbe1ea', padding: '6px 10px', fontWeight: 700, background: '#fff' }}>{copied ? 'Copied' : 'Copy key'}</button>
          </div>
        </div>
      ) : null}
      <form onSubmit={handleCreateApiKey} style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'end' }}>
        ...
        <button type="submit" disabled={loading || !apiKeyLabel.trim()} style={{ background: '#0f172a', color: '#fff', border: 'none', borderRadius: 12, padding: '11px 14px', fontWeight: 700 }}>
          {loading ? 'Creating…' : 'Create API key'}
        </button>
      </form>
      <div style={{ display: 'grid', gap: 8 }}>
        {apiKeys.map((key) => (
          <div key={key.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', padding: '12px', borderBottom: '1px solid #eef2f7' }}>
            <div style={{ display: 'grid', gap: 4 }}>
              <div style={{ fontWeight: 600 }}>{key.label}</div>
              <div style={{ color: '#64748b', fontSize: 13 }}>{key.prefix}… · created {new Date(key.createdAt).toLocaleString()}{key.lastUsedAt ? ` · last used ${new Date(key.lastUsedAt).toLocaleString()}` : ''}{key.revokedAt ? ' · revoked' : ''}</div>
            </div>
            <button onClick={() => void handleRevokeApiKey(key.id)} disabled={revokingId === key.id || !!key.revokedAt} style={{ borderRadius: 10, border: '1px solid #dbe1ea', padding: '6px 10px', fontWeight: 700, background: '#fff', color: '#0f172a' }}>
              {key.revokedAt ? 'Revoked' : revokingId === key.id ? 'Revoking…' : 'Revoke'}
            </button>
          </div>
        ))}
        {!apiKeys.length && !loading ? <div style={{ color: '#64748b', fontSize: 14 }}>No API keys yet.</div> : null}
      </div>
    </div>
  )
}
