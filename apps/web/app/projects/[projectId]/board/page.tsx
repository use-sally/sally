...
export default function ProjectBoardPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return (
    ...
    >
      ...
      {projectError ? <div style={{ color: '#991b1b', marginBottom: 16 }}>{projectError instanceof Error ? projectError.message : 'Failed to load project'}</div> : null}
      {boardError ? <div style={{ color: '#991b1b', marginBottom: 16 }}>{boardError instanceof Error ? boardError.message : 'Failed to load board'}</div> : null}
      {columns.length ? <TaskBoard columns={columns} taskBaseHref={`/projects/${projectId}/board`} projectId={projectId} /> : <div style={{ color: '#64748b' }}>{isLoading ? 'Loading board…' : 'No tasks yet.'}</div>}
