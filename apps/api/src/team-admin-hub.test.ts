...
test('team hub list is visible to platform admins and includes every account with workspace and project memberships', () => {
  ...
  assert.match(apiIndexSource, /prisma\.account\.findMany\(\{[\s\S]*include: \{[\s\S]*memberships:[\s\S]*projectMemberships:[\s\S]*twoFactorCredential: true[\s\S]*_count: \{ select: \{ webAuthnCredentials: true \} \}/)
  assert.match(apiIndexSource, /workspaceMemberships:[\s\S]*projectMemberships:/)
  assert.match(apiIndexSource, /twoFactorEnabled: Boolean\(account\.twoFactorCredential\?\.enabled \|\| account\._count\.webAuthnCredentials > 0\)/)
  assert.match(apiIndexSource, /twoFactorConfirmedAt: account\.twoFactorCredential\?\.confirmedAt\?\.toISOString\(\) \?\? null/)
  assert.match(apiIndexSource, /passkeyCount: account\._count\.webAuthnCredentials/)
})
