...
const hostedMcpTools: McpTool[] = [
  ...
  { name: 'project.dependencies.add', description: 'Add a project dependency. Both must be in the same workspace. Cycles are rejected.', inputSchema: { type: 'object', properties: { workspaceId: { type: 'string' }, workspaceSlug: { type: 'string' }, projectId: { type: 'string', description: 'The project that depends on another' }, dependsOnId: { type: 'string', description: 'The project it depends on' } }, required: ['projectId', 'dependsOnId'], additionalProperties: false } },
  { name: 'project.dependencies.remove', description: 'Remove a dependency between two projects.', inputSchema: { type: 'object', properties: { workspaceId: { type: 'string' }, workspaceSlug: { type: 'string' }, projectId: { type: 'string' }, dependsOnId: { type: 'string' } }, required: ['projectId', 'dependsOnId'], additionalProperties: false } },
  ...
  { name: 'task.get', description: 'Get full task details by ID.', inputSchema: { type: 'object', properties: { workspaceId: { type: 'string' }, workspaceSlug: { type: 'string' }, taskId: { type: 'string' } }, required: ['taskId'], additionalProperties: false } },
  { name: 'task.getByNumber', description: 'Get full task details by project-local number (e.g. #5).', inputSchema: { type: 'object', properties: { workspaceId: { type: 'string' }, workspaceSlug: { type: 'string' }, projectId: { type: 'string' }, number: { type: 'number' } }, required: ['projectId', 'number'], additionalProperties: false } },
  ...
  { name: 'task.dependencies.add', description: 'Add a task dependency. Both tasks must be in the same project. Cycles are rejected.', inputSchema: { type: 'object', properties: { workspaceId: { type: 'string' }, workspaceSlug: { type: 'string' }, taskId: { type: 'string', description: 'The task that depends on another' }, dependsOnId: { type: 'string', description: 'The task it depends on' } }, required: ['taskId', 'dependsOnId'], additionalProperties: false } },
  { name: 'task.dependencies.remove', description: 'Remove a dependency between two tasks.', inputSchema: { type: 'object', properties: { workspaceId: { type: 'string' }, workspaceSlug: { type: 'string' }, taskId: { type: 'string' }, dependsOnId: { type: 'string' } }, required: ['taskId', 'dependsOnId'], additionalProperties: false } },
...
async function callHostedMcpTool(request: any, name: string, args: Record<string, any>) {
  switch (name) {
    ...
    case 'project.dependencies.add':
      return await injectJson(request, { method: 'POST', url: `/projects/${args.projectId}/dependencies`, args, payload: { dependsOnId: args.dependsOnId } })
    case 'project.dependencies.remove':
      return await injectJson(request, { method: 'DELETE', url: `/projects/${args.projectId}/dependencies/${args.dependsOnId}`, args })
    ...
    case 'task.getByNumber': {
      const task = await prisma.task.findFirst({ where: { projectId: args.projectId, number: args.number, archivedAt: null } })
      if (!task) throw new Error(`Task #${args.number} not found in this project`)
      return await injectJson(request, { method: 'GET', url: `/tasks/${task.id}`, args })
    }
    case 'task.create':
    ...
    case 'task.dependencies.add':
      return await injectJson(request, { method: 'POST', url: `/tasks/${args.taskId}/dependencies`, args, payload: { dependsOnId: args.dependsOnId } })
    case 'task.dependencies.remove':
      return await injectJson(request, { method: 'DELETE', url: `/tasks/${args.taskId}/dependencies/${args.dependsOnId}`, args })
