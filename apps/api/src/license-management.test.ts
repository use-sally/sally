...
test('Admin mode exposes Edition\/License UI and web client functions', () => {
  ...
  assert.doesNotMatch(pageSource, /Refresh certificate|handleRefresh|refreshLicense\(/)
...
test('license auto-refresh is backend-driven for status checks, not a manual admin action', () => {
  const serviceSource = fs.readFileSync(licenseServicePath, 'utf8')
